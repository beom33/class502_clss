...
public class JoinValidator implements Validator<RequestJoin>,
   ...
   public void check(RequestJoin form) {
        ...
        checkRequried(email, new BadRequestException("이메일을 입력하세요."));
        checkRequried(password, new BadRequestException("비밀번호를 입력하세요."));
        checkRequried(confirmPassword, new BadRequestException("비밀번호를 확인하세요."));
        checkRequried(userName, new BadRequestException("회원명을 입력하세요."));
        checkTrue(termsAgree, new BadRequestException("약관에 동의하세요."));

        //비밀번호, 비밀번호 확인 일치 여부
        checkTrue(password.equals(confirmPassword), new
                BadRequestException("비밀번호가 일치하지 않습니다."));
        ...
        //비밀번호 자리수 체크
        checkTrue(password.length() >= 8, new BadRequestException
                ("비밀번호는 8자리 이상 입력하세요."));
