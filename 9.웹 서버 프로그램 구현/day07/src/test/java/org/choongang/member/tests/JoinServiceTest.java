...
public class JoinServiceTest {
    ...
    @Test
    @DisplayName("필수 입력항목(이메일, 비밀번호, 비밀번호 확인, 회원명, 약관동의, " +
            "검증, 검증 실패시 BadRequestException 발생")
    ...
    void requiredEachFieldTest(String field,boolean isnull, String
            keyword) {
    ...
    void passwordLengthTest() {
          BadRequestException thrown = assertThrows(BadRequestException
              ...
              RequestJoin form = getData();
              form.setPassword(faker.regexify("\\w{3,4}").toLowerCase());
              form.setConfirmPassword(form.getPassword());
              service.process(form);
          });
